...
module auth_path #(
    ...
    // =========================================================
    // Auth control signals in the clk domain
    // =========================================================
    ...
    // Use the saved grid ID for a grid-entry event.
    // Otherwise, use the current stable grid.
    assign control_grid_valid = grid_enter_clk ? 1'b1 : current_grid_valid_clk;

    assign control_grid_id =
        grid_enter_clk ? grid_event_id_sync : current_grid_id_clk;

    // Reset the grid stabilizer on start, clear, or success.
    // Keep it running after a normal pattern failure.
    assign analysis_clear_clk = i_auth_start | i_pattern_clear | o_unlock_pass;
    ...
    // Convert the clk-domain pulse to a toggle.
    ...
    // Restore the toggle as a pclk-domain pulse.
    always_ff @(posedge pclk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            o_path_clear_pclk <= path_clear_toggle_sync ^ path_clear_toggle_d;
    ...
    // =========================================================
    // Map the center point to a 3x3 grid.
    // =========================================================
    ...
    grid_stabilizer #(
        ...
        .i_clear(analysis_clear_pclk | ~auth_active_pclk),

        .i_grid_valid(mapped_grid_valid & auth_active_pclk),
    ...
    always_ff @(posedge pclk or negedge rst_n) begin
        ...
        end else if (stable_grid_enter_pclk && auth_active_pclk) begin
    ...
    logic grid_event_toggle_meta;
    logic grid_event_toggle_sync;
    ...
    // =========================================================
    // Transfer marker/grid state from pclk to clk.
    // =========================================================
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            grid_event_toggle_meta <= grid_event_toggle_pclk;

            grid_event_toggle_sync <= grid_event_toggle_meta;

            grid_event_toggle_d <= grid_event_toggle_sync;

            grid_enter_clk <= grid_event_toggle_sync ^ grid_event_toggle_d;

            grid_event_id_meta <= grid_event_id_pclk;

            grid_event_id_sync <= grid_event_id_meta;

            marker_valid_meta <= blob_valid_raw & auth_active_pclk;

            marker_valid_clk <= marker_valid_meta;

            current_grid_valid_meta <= stable_grid_valid_pclk;

            current_grid_valid_clk <= current_grid_valid_meta;

            current_grid_id_meta <= stable_grid_id_pclk;

            current_grid_id_clk <= current_grid_id_meta;
        end
    end
