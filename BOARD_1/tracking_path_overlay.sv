...
module tracking_path_overlay #(
    ...
    // Apply 1/2^SMOOTH_SHIFT of the position change.
    parameter integer SMOOTH_SHIFT       = 2,

    // Treat smaller movements as jitter.
    parameter integer MOVE_MARGIN        = 2,

    // Treat larger frame-to-frame jumps as false detections.
    parameter integer MAX_JUMP           = 80,

    // Line width is 2 * LINE_RADIUS + 1.
    parameter integer LINE_RADIUS        = 1,

    // Keep the line through this many missed frames.
    ...
    localparam integer PIXEL_COUNT = IMG_WIDTH * IMG_HEIGHT;

    localparam integer SMOOTH_ROUND = (SMOOTH_SHIFT == 0) ? 0 : (1 << (SMOOTH_SHIFT - 1));
    ...
    assign frame_sample = cam_vsync_d & ~i_cam_vsync;

    assign clear_pulse = i_clear & ~clear_d;
    ...
    function automatic integer smooth_delta(
        ...
        begin
            if (SMOOTH_SHIFT == 0) begin
                ...
                smooth_delta = (value + SMOOTH_ROUND) >>> SMOOTH_SHIFT;
            end else begin
                magnitude = -value;
                smooth_delta = -((magnitude + SMOOTH_ROUND)>>> SMOOTH_SHIFT);
            end
        end
    endfunction
    ...
    always_comb begin
        ...
        if (previous_valid) begin
            target_x_calc = previous_x + smooth_delta(delta_x_calc);

            target_y_calc = previous_y + smooth_delta(delta_y_calc);
        end else begin
            target_x_calc = raw_x_calc;
            target_y_calc = raw_y_calc;
        end

        // Clamp coordinates to the active video area.
        if (target_x_calc < 0)
            target_x_calc = 0;
        ...
        move_x_calc = abs_integer(target_x_calc - previous_x);

        move_y_calc = abs_integer(target_y_calc - previous_y);
        ...
        if (!i_rst_n) begin
            ...
            if (clear_pulse) begin
                ...
                case (state)
                    ...
                    //================================================
                    // Wait for a new center position.
                    //================================================
                    STATE_IDLE: begin
                        if (frame_sample) begin
                            if (i_marker_valid) begin
                                ...
                                if (!previous_valid) begin
                                    ...
                                    // Use the same start and end point
                                    // to draw a circle at the start.
                                    line_x     <= raw_x_calc;
                                    line_y     <= raw_y_calc;
                                ...
                                // Blue object briefly disappeared.
                                if (
                                    lost_frame_count + 1 >=
                                    LOST_FRAME_MARGIN
                                ) begin
                    ...
                    //================================================
                    // Add thickness around the current line pixel.
                    //================================================
                    STATE_STAMP: begin
                    ...
                    //================================================
                    // Calculate the next Bresenham line pixel.
                    //================================================
                    STATE_ADVANCE: begin
                        // Finish after drawing the end point.
                        if (
                            (line_x == line_end_x) &&
                            (line_y == line_end_y)
                        ) begin
    ...
    // Convert 640x480 coordinates to 320x240.
    //============================================================
    ...
    //============================================================
    // VGA Pipeline
    // Align video signals with the trail-buffer BRAM latency.
    //============================================================
