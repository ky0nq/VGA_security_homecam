...
module gauss_filter_pipe #(
    ...
    // if WEIGHT_SUM is a power of 2 (true whenever WIN itself is a power
    // of 2, e.g. WIN=16 -> 256), the divide below becomes a plain shift,
    // which is much cheaper in LUTs than a real divider. Both branches
    // give the identical answer for a power-of-2 WEIGHT_SUM (verified in
    // Python before adding this), so this is a free win, not a behavior
    // change.
    localparam bit IS_POW2    = (WEIGHT_SUM & (WEIGHT_SUM - 1)) == 0;
    localparam int NORM_SHIFT = $clog2(WEIGHT_SUM);
    ...
    generate
        for (r = 1; r < WIN; r = r + 1) begin : gen_linebuf
            ...
            // deliberately no reset (SRL inference) -- see note above LATENCY
    ...
    // ---------------- normalize (shift if possible, divide otherwise) + output register --
    function automatic logic [3:0] normalize(input logic [23:0] sum);
        logic [23:0] avg;
        if (IS_POW2) avg = sum >> NORM_SHIFT;   // cheap: just wiring, no LUTs
        else         avg = sum / WEIGHT_SUM;    // only used if WIN isn't a power of 2
