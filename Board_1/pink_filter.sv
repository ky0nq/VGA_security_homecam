...
// Color filter. Turns the picture pink when pink_en is high, passes it through otherwise.
...
module pink_filter_pipe (
    ...
    // Stage 2 : add up the brightness, then spread it into a pink tone, and pick output
    logic [11:0] y_sum;
    logic [ 3:0] gray;
    ...
    assign pink_r = gray;                // gray * 1.0 (dominant channel)
    assign pink_g = gray - (gray >> 2);  // gray * 0.75
    assign pink_b = gray - (gray >> 3);  // gray * 0.875
