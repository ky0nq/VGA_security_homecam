...
// Description: Applies a blue color filter (cool tone effect) to RGB444 video.
...
module blue_filter_pipe (
    ...
    // Stage 1: Calculate luminance weights
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            s1_r   <= 8'd77  * i_rgb[11:8];  // R weight (~0.3)
            s1_g   <= 8'd150 * i_rgb[7:4];   // G weight (~0.58)
            s1_b   <= 8'd29  * i_rgb[3:0];   // B weight (~0.11)
            s1_rgb <= i_rgb;
        end
    end

    // Stage 2: Convert to grayscale and apply blue tint
    logic [11:0] y_sum;
    logic [ 3:0] gray;
    ...
    assign blue_r = gray >> 2;             // 25% Red
    assign blue_g = gray - (gray >> 1);    // 50% Green
    assign blue_b = gray;                  // 100% Blue
    ...
    // Sync signal delay matching (2 cycles)
