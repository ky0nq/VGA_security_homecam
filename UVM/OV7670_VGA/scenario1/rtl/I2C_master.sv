...
module I2C_master #(
    ...
    parameter logic [7:0] DEVICE_ADDR = 8'h42    // Fixed SCCB write address
)(
    input  logic       clk,
    input  logic       rst_n,

    // One-cycle pulse from the OV7670 setup controller
    input  logic       start,
                    ...
                    if (clk_cnt == TICK_DIV - 1) begin
                        ...
                        case (step)
                            ...
                            2'd3 : begin
                                scl           <= 1'b0;
                                sda_drive_low <= 1'b1;
                                tx_shift_r <= DEVICE_ADDR;
                                bit_cnt <= 3'd0;
                                step    <= 2'd0;
                                state <= DEV_ADDR;
                            end
                            // Unknown phase: release the bus and return to idle.
                            // Normal phases 0..3 retain their original timing.
                            default : begin
                                state         <= IDLE;
                                busy          <= 1'b0;
                                done          <= 1'b0;
