...
module OV7670_setup_CNTL #(
    ...
    // OV7670 Reset Delay
    //
    // Wait 30 ms after writing 0x80 to 0x12.
    //============================================================
    ...
    // Write sequence checked with a logic analyzer
    //============================================================
    ...
    always_comb begin
        ...
        case (config_idx)
            ...
            // Entry 26 is 16'h0000 and intentionally uses the default below.
            // This shares identical ROM data without removing any table write.
            ...
            // Reset defaults target YUV and look less saturated in RGB565.
            // Apply these values after selecting RGB in COM7.
            //--------------------------------------------------------
            ...
            // Valid entry 26 and out-of-range indices both return 0000.
            default : begin
                reg_addr = 8'h00;
                reg_data = 8'h00;
            end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                //================================================
                // LOAD
                // Select reg_addr and reg_data from the LUT.
                //================================================
                LOAD : begin
                    state <= SEND;
                end
                ...
                //================================================
                // SEND
                // Wait until the I2C master is idle.
                // start = 1 clock
                //================================================
                SEND : begin
                    if (!i2c_busy) begin
                ...
                //================================================
                // DELAY
                // Wait 30 ms after the 0x12 reset command.
                //================================================
                DELAY : begin
