// Functional coverage records observed stimulus/results.  It does not decide
// correctness; the scoreboard and assertions do that independently.
class cam_vga_coverage extends uvm_subscriber #(cam_vga_seq_item);
    ...
    // VGA-only tests preload RAM directly.  This records applied memory
    // stimulus, not camera capture or proof that RAM stored the data correctly.
    ...
    covergroup camera_line_cg with function sample(int pixels, bit partial);
        ...
        // The supplied error sequence discards a single high byte.  It does
        // not append a partial byte after the 319/320/321-pixel line tests.
    ...
    covergroup camera_frame_cg with function sample(int lines);
        ...
        cp_lines: coverpoint lines {
            ...
            bins nominal = {240};
            // 239/241-line stress frames are not generated by this sequence.
            // Add explicit bins with their tests if that scope is extended.
        }
    endgroup
    ...
    // A phase bin records exposure, not a timing/data correctness verdict.
    ...
    // It is deliberately not described as a complete-frame correctness pass.
    ...
    // period by the monitor's independent cadence, not once per sysclk.
    ...
    // A HSYNC low level seen once does not cover the 96-pixel requirement.
