...
// Color filter. Turns the picture orange when orange_en is high, passes it through otherwise.
...
module orange_filter_pipe (
    ...
    // Stage 2 : add up the brightness, then spread it into an orange tone, and pick output
    logic [11:0] y_sum;
    logic [ 3:0] gray;
    ...
    assign orange_r = gray;                // gray * 1.0 (dominant channel)
    assign orange_g = gray - (gray >> 2);  // gray * 0.75
    assign orange_b = gray >> 3;           // gray * 0.125, almost none, keeps it warm
